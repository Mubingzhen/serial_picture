...
# 计算需要传输的数据包数量
num_packets = (width * height * 3 + 149 - 1) // 149
print(f'num_packets={num_packets}')
# 创建二进制文件并写入宽度和高度的值
with open('方案4_output.bin', 'wb') as f:
...
file_size = os.path.getsize('方案4_output.bin')
print(f'file_size={file_size}')
last_packet_size = file_size % 149
...
if len(packed_data) < 300:
    packed_data += b'\x00' * (300 - len(packed_data))
...
# 打开二进制文件，创建缓冲区
with open('方案4_output.bin', 'rb') as f:
    # 循环发送数据
