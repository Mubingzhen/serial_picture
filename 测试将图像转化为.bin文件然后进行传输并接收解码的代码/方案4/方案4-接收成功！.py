...
while True:
    ...
    break'''
data1=ser.read(300)
num_packets1, last_packet_size1=struct.unpack('ii',data1[:8])
print(f"num_packets1={num_packets1}, last_packet_size1={last_packet_size1}")
ser.write(b'A')
ser.write(b'A')
# 创建二进制文件并写入宽度和高度的值
with open('received_output.bin', 'wb') as f:
    ...
    for i in range(num_packets1):
        if i < num_packets1 - 1 :
            ...
            # 等待发送端发送数据包
            while True:
                if ser.in_waiting >= 149:
                    data = ser.read(150)
                    break

            # 解析数据包
            checksum = sum(data[:149]) & 0xff
            if checksum == data[149]:
                f.write(data[:149])
                ser.write(b'A')
...
# 保存还原的图像
img.save('方案4_restored_image.jpg')