            ...
            while True:
                if self.exit_flag == False:
                    ...
                    if self.serial_port.in_waiting > 0:
                        ...
                        if self.buffer_size is None and len(data)>=2:
                            # 解析前两个字节作为buffer_size的值
                            self.buffer_size = struct.unpack('>H', data[:2])[0]
                        ...
                        if self.buffer_size is not None and self.buffer.tell()>=self.buffer_size+2:
                        #if self.buffer.tell() >= 1113505:  # 如果接收到的数据长度达到图片的长度
                            self.buffer.seek(2)  # 将缓存指针移到起始位置
